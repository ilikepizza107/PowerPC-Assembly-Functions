...
void ClearASLData()
{
	...
	SetRegister(Reg2, 0);
	StoreHalfToHeapAddress(HeapType::Replay, Reg2, Reg1, REPLAY_HEAP_ALT_STAGE_STORAGE_OFF);
...
void AlternateStageFix()
{
	...
	LoadHalfFromHeapAddress(HeapType::Replay, 3, 12, REPLAY_HEAP_ALT_STAGE_STORAGE_OFF); //equals 0 if in a match
	OR(3, 3, 5);
...
void RecordInput()
{
	...
	StoreHalfToHeapAddress(HeapType::Replay, Reg1, Reg2, REPLAY_HEAP_ALT_STAGE_STORAGE_OFF);
...
void MakeBrawlSaveWholeReplay()
{
	...
	ADDI(5, 5, 0xA);
	GetHeapAddress(HeapType::Replay, 30, 30);
